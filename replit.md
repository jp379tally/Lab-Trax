...
- `PLATFORM_ADMIN_SECRET` — **required in production**; a strong secret string that must be sent as `X-Platform-Admin-Secret` header to access all `/api/admin/*` platform-wide endpoints (backup, cleanup, system settings). If unset, all admin endpoints return 403. Used by the Electron desktop bridge (stored in the OS keychain) and by CI workflows.
- `PLATFORM_ADMIN_PIN` — optional short numeric PIN (e.g. 4–6 digits) accepted as an alternative to `PLATFORM_ADMIN_SECRET` via the `X-Platform-Admin-Pin` header. Only valid for human admins — `isPlatformAdmin()` still requires a signed-in `role:"admin"` user, so the PIN alone cannot authenticate. The CI/automation path (`platformAdminUserOrSecret`) intentionally accepts the long secret only. The web unlock modal (LabTrax Desktop renderer in the browser preview) prompts for this PIN.
