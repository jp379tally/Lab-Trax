...
- **Hourly Server-Side Scheduler**: `server/index.ts` schedules an automatic backup 2 minutes after server start, then every hour — runs whether or not any user has the app open. Automatically tiers backups: daily (Mon–Sat), weekly (Sunday), monthly (1st of month). Saves to BOTH local `backups/{tier}/` folder AND OneDrive `LabTrax Backups/{tier}/` subfolder. If OneDrive fails, local copy is preserved. Retention: 7 daily, 4 weekly, 3 monthly.
- **Restore Endpoint**: `POST /api/admin/restore` — admin-only. Accepts a backup ZIP file upload with `confirmation: "RESTORE"` and `mode: "merge"|"replace"`. Extracts and restores all tables: cases, organizations, memberships, invoices, line items, payments. `merge` mode uses `onConflictDoNothing` (safe import). `replace` mode deletes existing data first. Logs the restore action to audit_logs. Returns `{ success, mode, backupDate, restored: { counts } }`.
- **Backup List**: `GET /api/admin/backups/list` — returns all locally stored backups grouped by tier (daily/weekly/monthly) with name, size, and createdAt.
- **Backup Download**: `GET /api/admin/backups/download/:tier/:filename` — streams a specific local backup file for download.
