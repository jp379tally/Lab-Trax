...
- **Source of truth**: `lab_cases.organization_id` (indexed). A startup migration in `server/index.ts` backfills this column from the `affiliationKey`/`affiliationName` JSON fields for legacy rows where it is still NULL. The backfill is **owner-membership-gated**: it only writes a non-null organization_id if (a) the target lab actually exists and (b) the case's owner is currently an active member of that lab. This makes the migration purely restorative — a stale `affiliationKey` left over from a lost membership cannot silently re-share a private case.
- **Migration safety**: A PL/pgSQL helper `try_to_jsonb(text)` returns NULL on parse failure so a single malformed legacy row cannot abort the backfill. A REMEDIATION pass also runs first, NULLing any existing organization_id that points to a deleted org or a lab the case's owner has left, so corruption from older migration versions self-heals on next boot.
...
- **`POST /api/legacy/cases`** wraps the entire authz check + upsert in a single transaction with `SELECT ... FOR UPDATE` row lock to close the check-then-write TOCTOU window. Inserts force `ownerId = caller`; updates require either existing-owner or active member of the case's current lab. The body's ownerId is ignored on writes. Cross-lab affiliationKey is silently downgraded to private (`organization_id = NULL`) via `server/lib/case-visibility.ts → resolveOrganizationIdForWrite`.
...
- **Tests**: `server/__tests__/case-visibility.test.ts` (vitest) covers the visibility rule and cross-lab write protection. Run with `npx vitest run`.
