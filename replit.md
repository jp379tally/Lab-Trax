...
- **Server-Synced Cases**: Cases are stored both locally (AsyncStorage) and on the server (`lab_cases` PostgreSQL table). When a case is created or modified, it's automatically synced to the server. On login, cases are fetched from the server for all group members and merged with local data. This enables case sharing across devices for users in the same lab group.
- **Group-Based Case Sharing**: Cases are visible to all members of the same lab group, not just the creator. The `groupMemberIds` memo collects all user IDs from the current user's groups, and `cases` filters `allCases` to show cases owned by any group member. Group member IDs are auto-repaired on load to match real auth IDs from the server.
- **Data Isolation (HIPAA)**: Cases are filtered by group membership (`groupMemberIds`). The `currentUserId` is exposed from `AuthContext` and consumed by `AppProvider` for case ownership filtering and stamping. Users not affiliated with any group see an informational banner but can still access their own cases. Profile pictures are stored per-user (keyed by user ID) and cleared on logout.
