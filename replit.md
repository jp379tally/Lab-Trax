...
- **Smile Preview AR**: Provider portal has an AR teeth whitening preview feature at `/smile-preview`. Uses MediaPipe Face Mesh for real-time face landmark detection and overlays whitened teeth visualization. The HTML page is served by Express from `server/templates/smile-preview.html` and loaded via WebView (native) or iframe (web) in `app/smile-preview.tsx`. Supports shade selection (A1, A2, BL1-3, B1, Hollywood), brightness control, toggle on/off, and photo capture.