...
- **Nightly Server-Side Scheduler**: `server/index.ts` schedules an automatic backup 2 minutes after server start, then every 24 hours — runs whether or not any user has the app open. Automatically tiers backups: daily (Mon–Sat), weekly (Sunday), monthly (1st of month). Saves to BOTH local `backups/{tier}/` folder AND OneDrive `LabTrax Backups/{tier}/` subfolder. If OneDrive fails, local copy is preserved. Retention: 7 daily, 4 weekly, 3 monthly.
