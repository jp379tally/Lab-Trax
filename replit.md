...
- **Domain rule (lab inbox model)**: A scanner is allowed to drop a case into ANY lab's inbox, even if they are not a member of that lab — same way a dentist can mail prescriptions to a fulfillment lab. Once a case is tagged with an `affiliationKey: "org:<UUID>"`, every active member of that lab sees it in their case list, regardless of who scanned it. This is enforced on writes (POST), reads (GET), and the startup migration backfill — none of them require the case's owner to be a member of the receiving lab.
- **Source of truth**: `lab_cases.organization_id` (indexed). A startup migration in `server/index.ts` backfills this column from the `affiliationKey`/`affiliationName` JSON fields for legacy rows where it is still NULL. The only safety rail is that the target lab must exist as `type='lab'` — we never write a reference to a phantom org. There is NO owner-membership requirement (an earlier version had one and it silently hid valid lab cases whose scanner wasn't in the lab; that regression is what this change fixes).
- **Migration safety**: A PL/pgSQL helper `try_to_jsonb(text)` returns NULL on parse failure so a single malformed legacy row cannot abort the backfill. A REMEDIATION pass also runs first, NULLing any organization_id that points to a non-existent or non-lab org so true corruption self-heals on next boot.
...
- **`POST /api/legacy/cases`** wraps the entire authz check + upsert in a single transaction with `SELECT ... FOR UPDATE` row lock to close the check-then-write TOCTOU window. Inserts force `ownerId = caller`; updates require either existing-owner or active member of the case's current lab. The body's ownerId is ignored on writes. The `affiliationKey` is parsed by `server/lib/case-visibility.ts → parseOrganizationIdFromAffiliationKey` (a pure, permissive parser); the route then verifies the org exists as `type='lab'` before persisting it. Cross-lab tagging is allowed by design; tagging a non-existent org is silently downgraded to a private case.
...
- **Tests**: `server/__tests__/case-visibility.test.ts` (vitest, 14 tests) covers the visibility rule and the permissive write parser, including the explicit "non-member can drop into a lab inbox" case. Run with `npx vitest run server/__tests__/case-visibility.test.ts`.
