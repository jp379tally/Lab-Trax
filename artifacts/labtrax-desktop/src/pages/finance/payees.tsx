...
import { AlertTriangle, GitMerge, Loader2, Pencil, Plus, Search, Trash2, X } from "lucide-react";
...
function MergeDialog({
  ...
  async function loadPreview(id: string) {
    ...
    setPreview(null);
    try {
      const result = await apiFetch<MergePreview>(`/finance/vendors/${source.id}/merge`, {
        method: "POST",
  ...
  const canonicalVendor = candidates.find((v) => v.id === canonicalId);
  const totalRelinked = preview ? preview.transactionsRelinked + preview.recurringRelinked : 0;
      ...
      <div className="bg-card border border-border rounded-xl shadow-xl w-full max-w-md mx-4">
        ...
        <div className="px-5 py-4 space-y-4">
          <div className="rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 px-3 py-2.5 text-sm">
            <p className="font-medium text-amber-800 dark:text-amber-200">
              Merging: <span className="font-semibold">{source.name}</span>
            </p>
            <p className="text-amber-700 dark:text-amber-300 text-xs mt-0.5">
              All transactions linked to this payee will be re-assigned to the one you choose below, and this payee will be removed.
            </p>
          </div>
          ...
          {preview && canonicalVendor && (
            <div className="rounded-lg bg-secondary/40 border border-border px-3 py-3 space-y-1">
              ...
              <ul className="text-sm text-muted-foreground space-y-0.5 mt-1">
                <li>
                  <span className="text-foreground font-medium">{preview.transactionsRelinked}</span>{" "}
                  {preview.transactionsRelinked === 1 ? "transaction" : "transactions"} will be re-linked to{" "}
                  <span className="font-medium text-foreground">{canonicalVendor.name}</span>
                </li>
                {preview.recurringRelinked > 0 && (
                  <li>
                    <span className="text-foreground font-medium">{preview.recurringRelinked}</span>{" "}
                    recurring {preview.recurringRelinked === 1 ? "rule" : "rules"} will be re-linked
                  </li>
                )}
                {totalRelinked === 0 && (
                  <li className="text-muted-foreground">No transactions are linked to this payee.</li>
                )}
              </ul>
              <p className="text-xs text-muted-foreground pt-1">
                This action cannot be undone. <span className="font-medium text-foreground">{source.name}</span> will be permanently removed.
              </p>
            </div>
          )}
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-border">
          ...
          <button
            ...
            disabled={!canonicalId || !preview || previewLoading || mergeMut.isPending}
          ...
          >
            {mergeMut.isPending && <Loader2 size={13} className="animate-spin" />}
                      ...
                      <td className="px-4 py-2.5 font-medium">
                        <span className="inline-flex items-center gap-1.5">
                          ...
                          {duplicateNames.has(v.name.toLowerCase()) && (
                            <span
                              title="Duplicate name — another payee in this lab shares this name. Use 'Merge into…' to resolve."
                        ...
                        <div className="flex items-center justify-end gap-0.5">
                          {duplicateNames.has(v.name.toLowerCase()) && (
                            <button
                              ...
                              className="h-7 w-7 rounded-md hover:bg-amber-100 dark:hover:bg-amber-900/30 flex items-center justify-center text-amber-600 dark:text-amber-400 hover:text-amber-700"
