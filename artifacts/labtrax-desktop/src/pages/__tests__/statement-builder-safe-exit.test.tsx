...
import { StatementBuilderDialog } from "@/pages/invoices";
...
function renderDialog() {
  ...
  render(
    <Wrapper>
      <StatementBuilderDialog knownLabOrgId="lab-1" onClose={onClose} />
    </Wrapper>,
  );
  return { onClose };
}
...
function selectPractice() {
  // Selecting a practice is the primary "unsaved input" for the builder.
  fireEvent.change(screen.getByDisplayValue("Select practice…"), {
    target: { value: "prov-1" },
  });
}

describe("StatementBuilderDialog safe-exit", () => {
  ...
  it("'Keep editing' cancels the prompt and leaves the dialog open", async () => {
    ...
    expect(screen.queryByText("Discard changes?")).not.toBeInTheDocument();
    expect(onClose).not.toHaveBeenCalled();
    // The selected practice is still present — nothing was lost.
    expect(screen.getByDisplayValue("Dr. Smith Practice")).toBeInTheDocument();
  });
  ...
  it("treats a practice selected then cleared back to none as clean", async () => {
    const { onClose } = renderDialog();
    await waitForLoaded();

    const select = screen.getByDisplayValue("Select practice…");
