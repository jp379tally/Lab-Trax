...
import {
  ...
  corrDefaultTextBlockPosition,
  DEFAULT_BODY_TEXT,
  DEFAULT_CLOSING_TEXT,
  DEFAULT_CORRESPONDENCE_TEMPLATE,
  type CorrespondenceDefaultTextBlock,
  type CorrespondenceTemplate,
  type CorrespondenceTemplateBox as TemplateBox,
  type CorrespondenceTextBlock,
  type TextAlign,
} from "@/lib/correspondence-template";
import { previewCorrespondencePdf } from "@/lib/export";

const PAGE_W = 612;
const PAGE_H = 792;
const FONT_SIZES = [8, 10, 12, 14, 18] as const;

type SectionKey = keyof CorrespondenceTemplate["boxes"];

interface TemplateApi {
  template: CorrespondenceTemplate;
  isCustom: boolean;
  defaultTemplate: CorrespondenceTemplate;
}
...
function applyDrag(start: TemplateBox, h: Handle, dx: number, dy: number): TemplateBox {
  ...
  w = Math.max(20, w); bh = Math.max(20, bh);
  x = clamp(x, 0, PAGE_W - w); y = clamp(y, 0, PAGE_H - bh);
  return { x: Math.round(x), y: Math.round(y), w: Math.round(w), h: Math.round(bh) };
}

function newTextBlock(): CorrespondenceTextBlock {
  return { id: crypto.randomUUID(), x: 40, y: 680, w: 240, h: 40, text: "", fontSize: 10, align: "left", bold: false };
}

function isDefaultEnabled(draft: CorrespondenceTemplate, defId: string): boolean {
  return draft.customTexts.some((ct) => ct.sourceId === defId);
}

function enableDefault(draft: CorrespondenceTemplate, def: CorrespondenceDefaultTextBlock): CorrespondenceTemplate {
  if (isDefaultEnabled(draft, def.id)) return draft;
  const idx = draft.defaultTextBlocks.findIndex((d) => d.id === def.id);
  const pos = corrDefaultTextBlockPosition(idx >= 0 ? idx : draft.defaultTextBlocks.length);
  const newBlock: CorrespondenceTextBlock = {
...
function disableDefault(draft: CorrespondenceTemplate, defId: string): CorrespondenceTemplate {
  return { ...draft, customTexts: draft.customTexts.filter((ct) => ct.sourceId !== defId) };
}

function syncDefaultToCanvas(
  draft: CorrespondenceTemplate,
  patch: Partial<CorrespondenceDefaultTextBlock> & { id: string },
...
export function CorrespondenceLayoutPanel() {
  ...
  const query = useQuery<TemplateApi>({
    enabled: !!orgId,
    queryKey: ["correspondenceTemplate", orgId],
    queryFn: async () => {
      const res = await apiFetch<{ data: TemplateApi } | TemplateApi>(
        `/organizations/${orgId}/correspondence-template`,
      );
      return (res as any).data ?? (res as any);
    },
  });

  const [draft, setDraft] = useState<CorrespondenceTemplate>(DEFAULT_CORRESPONDENCE_TEMPLATE);
  const [dirty, setDirty] = useState(false);
  const [selected, setSelected] = useState<{ kind: "section" | "logo" | "extra" | "text"; key: string } | null>(null);
  const [isPreviewing, setIsPreviewing] = useState(false);
  const [previewError, setPreviewError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<"canvas" | "body" | "closing">("canvas");
  ...
  useEffect(() => {
    ...
    setDraft(coerceCorrespondenceTemplate(query.data.template));
    setDirty(false);
  }, [query.data]);
  ...
  const saveMutation = useMutation({
    mutationFn: async (template: CorrespondenceTemplate | null) => {
      await apiFetch(`/organizations/${orgId}/correspondence-template`, {
        method: "PUT",
        body: JSON.stringify({ template }),
      });
    },
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["correspondenceTemplate", orgId] });
      void refresh?.();
    },
  });
  ...
  function onPointerMove(e: React.PointerEvent) {
    ...
    setDraft((cur) => {
      ...
      if (d.kind === "extra") {
        const idx = Number(d.key); const extras = cur.extraImages.slice();
        extras[idx] = { ...extras[idx], ...next }; return { ...cur, extraImages: extras };
      }
      if (d.kind === "text") {
  ...
  function updateSelectedText(patch: Partial<CorrespondenceTextBlock>) {
  ...
  function insertMergeField(field: string, target: "body" | "closing") {
    if (target === "body") {
      setDraft((d) => ({ ...d, bodyText: d.bodyText + field }));
    } else {
      setDraft((d) => ({ ...d, closingText: d.closingText + field }));
    }
    setDirty(true);
  }
  ...
  async function handlePreview() {
    ...
    try {
      ...
      const opened = previewCorrespondencePdf({
  ...
  function addDefaultBlock() {
    const newDef: CorrespondenceDefaultTextBlock = { id: crypto.randomUUID(), text: "", fontSize: 10, align: "left", bold: false };
    setDraft((d) => { const withDef = { ...d, defaultTextBlocks: [...d.defaultTextBlocks, newDef] }; return enableDefault(withDef, newDef); });
    setEditingDefaultId(newDef.id);
    setDirty(true);
  }

  function updateDefaultBlock(id: string, patch: Partial<CorrespondenceDefaultTextBlock>) {
    setDraft((d) => {
  ...
  function deleteDefaultBlock(id: string) {
    setDraft((d) => ({ ...d, defaultTextBlocks: d.defaultTextBlocks.filter((b) => b.id !== id), customTexts: d.customTexts.filter((ct) => ct.sourceId !== id) }));
    setDirty(true);
    if (editingDefaultId === id) setEditingDefaultId(null);
  }

  function toggleDefault(def: CorrespondenceDefaultTextBlock) {
  ...
  function moveDefaultBlock(id: string, dir: "up" | "down") {
    setDraft((d) => {
      ...
      const customTexts = d.customTexts.map((ct) => {
        ...
        const pos = corrDefaultTextBlockPosition(newIdx);
  ...
  if (!orgId) {
    return <div className="p-6 text-sm text-muted-foreground">Select a lab organization to edit its correspondence layout.</div>;
  }
  if (query.isLoading) {
  ...
  if (query.isError) {
    return <div className="p-6 text-sm text-destructive">Failed to load correspondence template. {(query.error as Error)?.message}</div>;
  }
  ...
  return (
    <div className="p-6">
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          <h2 className="text-lg font-semibold">Correspondence layout</h2>
          <p className="text-sm text-muted-foreground mt-1">
            Design the correspondence letter PDF layout. Use merge fields in the body text to personalize each letter.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button type="button"
            onClick={() => { setDraft(coerceCorrespondenceTemplate(query.data?.template)); setDirty(false); setSelected(null); setEditingDefaultId(null); }}
          ...
          <button type="button"
            onClick={() => {
              ...
              const freshTexts: CorrespondenceTextBlock[] = preserved.map((def, i) => ({
                id: crypto.randomUUID(), sourceId: def.id, ...corrDefaultTextBlockPosition(i),
                text: def.text, fontSize: def.fontSize, align: def.align, bold: def.bold,
              }));
              saveMutation.mutate({ ...DEFAULT_CORRESPONDENCE_TEMPLATE, defaultTextBlocks: preserved, customTexts: freshTexts });
            }}
      ...
      {/* Sub-tabs: Canvas / Body / Closing */}
      <div className="flex gap-1 mb-4 border-b border-border">
        {(["canvas", "body", "closing"] as const).map((t) => (
          <button key={t} type="button" onClick={() => setActiveTab(t)}
            ...
            {t === "canvas" ? "Layout canvas" : t === "body" ? "Body text" : "Closing / Signature"}
          </button>
        ))}
      </div>
        ...
        <div className="grid grid-cols-[1fr_280px] gap-6">
          ...
          >
            ...
            {(Object.keys(draft.boxes) as SectionKey[]).map((k) => (
              <DraggableBox key={k} box={draft.boxes[k]} color={CORR_SECTION_COLORS[k]}
                label={CORR_SECTION_LABELS[k]}
          ...
          <div className="space-y-5 text-sm">
            {selectedText && !selectedText.sourceId ? (
              <section className="p-3 rounded border border-orange-200 bg-orange-50 dark:bg-orange-950/20 dark:border-orange-800 space-y-2">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-orange-600 dark:text-orange-400 mb-2">Edit text block</h3>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button type="button" onClick={() => deleteTextBlock(selectedText.id)} className="p-1.5 rounded text-destructive hover:bg-destructive/10"><Trash2 size={12} /></button>
                </div>
              </section>
            ) : null}
            ...
            <section>
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2">
                  <input type="radio" name="corrLogoMode" checked={draft.logo.mode === "header"}
                    onChange={() => { setDraft((d) => ({ ...d, logo: { ...d.logo, mode: "header" } })); setDirty(true); }} />
                  Header (top-right)
                </label>
                <label className="flex items-center gap-2">
                  <input type="radio" name="corrLogoMode" checked={draft.logo.mode === "watermark"}
                ...
                <ul className="mt-3 space-y-1">
                  {draft.customTexts.filter((ct) => !ct.sourceId).map((tb, i) => (
                    <li key={tb.id}
                      ...
                      <button type="button" onClick={(e) => { e.stopPropagation(); deleteTextBlock(tb.id); }} className="text-destructive hover:bg-destructive/10 p-0.5 rounded"><X size={11} /></button>
                    </li>
                  ))}
                </ul>
            ...
            <section>
              ...
              <label className="block">
                ...
                <input type="file" accept="image/*" onChange={(e) => { const f = e.currentTarget.files?.[0]; if (f) uploadMutation.mutate(f); e.currentTarget.value = ""; }} className="hidden" id="corr-extra-upload" />
                <button type="button" onClick={() => document.getElementById("corr-extra-upload")?.click()}
            ...
            <section>
              ...
              <ul className="space-y-1 text-xs">
                {(Object.keys(CORR_SECTION_LABELS) as SectionKey[]).map((k) => (
                  <li key={k} className="flex items-center gap-2">
                    <span className="inline-block w-3 h-3 rounded-sm border border-border" style={{ background: CORR_SECTION_COLORS[k] }} />
                    {CORR_SECTION_LABELS[k]}
                  </li>
                ))}
              ...
              {overlappingPairs.length > 0 ? (
                <div className="flex items-start gap-1.5 rounded px-2 py-1.5 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-400 text-xs mt-3">
                  ...
                  <div>
                    ...
                    <ul className="space-y-0.5">{overlappingPairs.map(([a, b]) => <li key={`${a}-${b}`}>{CORR_SECTION_LABELS[a]} overlaps {CORR_SECTION_LABELS[b]}</li>)}</ul>
                  </div>
                </div>
              ) : null}
              {outOfBoundsSections.length > 0 ? (
                <div className="flex items-start gap-1.5 rounded px-2 py-1.5 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-400 text-xs mt-3">
                  ...
                  <div>
                    ...
                    <ul className="space-y-0.5">{outOfBoundsSections.map(({ key, edges }) => <li key={key}>{CORR_SECTION_LABELS[key]} extends past the {edges.join(" and ")} of the page</li>)}</ul>
                  </div>
                </div>
              ) : null}
                    ...
                    return (
                      <li key={def.id} className="rounded border border-border bg-background overflow-hidden"
                        ...
                        <div className="flex items-center gap-1.5 px-2 py-1.5">
                          <button type="button" onClick={() => toggleDefault(def)}
                          ...
                          <span className="flex-1 text-xs truncate text-muted-foreground min-w-0">{def.text || <span className="italic">Empty snippet</span>}</span>
                          ...
                          <button type="button" onClick={() => setPinnedPreviewId(pinnedPreviewId === def.id ? null : def.id)}
                          ...
                          <button type="button" onClick={() => { const opening = editingDefaultId !== def.id; setEditingDefaultId(opening ? def.id : null); if (opening) setPinnedPreviewId(null); }}
                          ...
                          <button type="button" onClick={() => { if (!confirm("Delete this default text block?")) return; deleteDefaultBlock(def.id); }}
                        ...
                        {!isEditing && (hoveredDefaultId === def.id || pinnedPreviewId === def.id) ? (
                          <div className="border-t border-border px-3 py-2 bg-muted/30">
                            {def.text.trim() ? (
                              <div style={{ fontSize: def.fontSize, fontWeight: def.bold ? "bold" : "normal", textAlign: def.align as TextAlign, lineHeight: 1.4, whiteSpace: "pre-wrap", wordBreak: "break-word", userSelect: "none" }}>{def.text}</div>
                          ...
                          <div className="px-2 pb-2 space-y-2 border-t border-border pt-2">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <button type="button" onClick={() => setEditingDefaultId(null)}
                                className="text-xs px-2 py-1 rounded bg-primary/10 text-primary hover:bg-primary/20 flex items-center gap-1"><Check size={11} /> Done</button>
                            </div>
                          </div>
                        ) : null}
                      </li>
        ...
        <div className="space-y-4 max-w-2xl">
          <div>
            <p className="text-xs text-muted-foreground mb-3">
              Write the body of the letter. Use merge-field chips below to insert placeholders that are filled in when the letter is generated.
            </p>
            <div className="flex flex-wrap gap-1.5 mb-3">
              {CORRESPONDENCE_MERGE_FIELDS.map((f) => (
                <button key={f.key} type="button" onClick={() => insertMergeField(f.key, "body")}
                  className="text-xs px-2 py-1 rounded-full border border-primary/30 bg-primary/5 text-primary hover:bg-primary/10 font-mono">
                  {f.key} <span className="text-muted-foreground font-sans">({f.label})</span>
                </button>
              ))}
            </div>
            <textarea
              className="w-full text-sm p-3 rounded border border-border bg-background resize-y focus:outline-none focus:ring-1 focus:ring-primary font-mono leading-relaxed"
              rows={16}
              placeholder="Enter body text with merge fields…"
              value={draft.bodyText}
              onChange={(e) => { setDraft((d) => ({ ...d, bodyText: e.target.value })); setDirty(true); }}
            />
          </div>
          <div className="rounded border border-border bg-muted/30 p-3">
            <p className="text-xs font-semibold text-muted-foreground mb-1">Sample preview (resolved)</p>
            <div className="text-sm whitespace-pre-wrap leading-relaxed text-foreground">
              {draft.bodyText
                .replace(/\{\{practiceName\}\}/g, "Riverside Dental")
                .replace(/\{\{patientName\}\}/g, "Jane Doe")
                .replace(/\{\{balance\}\}/g, "$1,400.00")
                .replace(/\{\{dueDate\}\}/g, "June 1, 2026")
                .replace(/\{\{labName\}\}/g, "Sample Dental Lab")
                .replace(/\{\{date\}\}/g, new Date().toLocaleDateString("en-US"))
                .replace(/\{\{invoiceNumber\}\}/g, "INV-042")}
            </div>
          </div>
          <button type="button" onClick={() => { setDraft((d) => ({ ...d, bodyText: DEFAULT_BODY_TEXT })); setDirty(true); }}
            className="text-xs px-3 py-1.5 rounded border border-border hover:bg-secondary flex items-center gap-1 text-muted-foreground">
            <RotateCcw size={11} /> Reset to default text
          </button>
        </div>
      ) : (
        <div className="space-y-4 max-w-2xl">
          <div>
            <p className="text-xs text-muted-foreground mb-3">
              Write the closing and signature block (e.g. "Sincerely," followed by the lab name).
            </p>
            <div className="flex flex-wrap gap-1.5 mb-3">
              {CORRESPONDENCE_MERGE_FIELDS.map((f) => (
                <button key={f.key} type="button" onClick={() => insertMergeField(f.key, "closing")}
                  className="text-xs px-2 py-1 rounded-full border border-primary/30 bg-primary/5 text-primary hover:bg-primary/10 font-mono">
                  {f.key} <span className="text-muted-foreground font-sans">({f.label})</span>
                </button>
              ))}
            ...
            <textarea
              ...
              onChange={(e) => { setDraft((d) => ({ ...d, closingText: e.target.value })); setDirty(true); }}
            />
          </div>
          <div className="rounded border border-border bg-muted/30 p-3">
            <p className="text-xs font-semibold text-muted-foreground mb-1">Sample preview (resolved)</p>
          ...
          <button type="button" onClick={() => { setDraft((d) => ({ ...d, closingText: DEFAULT_CLOSING_TEXT })); setDirty(true); }}
            className="text-xs px-3 py-1.5 rounded border border-border hover:bg-secondary flex items-center gap-1 text-muted-foreground">
            <RotateCcw size={11} /> Reset to default closing
          </button>
        </div>
      )}
    </div>
...
interface DraggableBoxProps {
  ...
  imageUrl?: string;
  opacity?: number;
  textBlock?: CorrespondenceTextBlock;
...
function DraggableBox({ box, color, label, selected, overlapping, imageUrl, opacity, textBlock, onStart, onDelete }: DraggableBoxProps) {
  ...
  return (
    <div style={style} onPointerDown={(e) => onStart(e, "move")} onPointerEnter={() => setHovered(true)} onPointerLeave={() => setHovered(false)}>
      ...
      {textBlock && !imageUrl ? (
        <div style={{ position: "absolute", inset: 0, overflow: "hidden", pointerEvents: "none", padding: "2px" }}>
          {textBlock.text?.trim() ? (
            <span style={{ fontSize: `${textBlock.fontSize * 0.35}px`, fontWeight: textBlock.bold ? "bold" : "normal", textAlign: textBlock.align, color: "#0f172a", lineHeight: 1.3, whiteSpace: "pre-wrap", wordBreak: "break-word", display: "block" }}>
              {textBlock.text}
            </span>
