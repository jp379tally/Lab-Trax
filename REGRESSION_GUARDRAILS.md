...
| Layer | File | What it guards |
|-------|------|----------------|
| Desktop unit | `artifacts/labtrax-desktop/src/components/__tests__/AuthedMedia.attachment.test.tsx` | Case-attachment photos render through `AuthedImage` — the shared component behind every desktop attachment surface (AttachmentRow, Files tab, image lightbox, history-event thumbnails, PrescriptionPreview). Two layers: **(A) the shared `AuthedImage` component** guards that it (1) fetches the canonical bearer-auth-gated `GET /api/cases/:caseId/attachments/:attachmentId/file` via `authedFetch` and renders the resulting object URL; (2) the rendered `<img src>` is the blob object URL, **never** the raw protected `/file` URL (a plain `<img src="…/file">` would 401 and render blank — the stale-desktop-build bug this locks against); (3) the fallback (not an `<img>`) renders when the authenticated fetch fails. **(B) the real Files-tab attachment surface** mounts the actual shipped `AttachmentThumb` (from `PrescriptionPreview.tsx`) with a mock image attachment and asserts it builds the canonical `/file` URL and renders it *through* `AuthedImage` (object-URL `<img>`, never a plain `<img src="…/file">`, plus the "Unavailable" fallback on fetch failure) — so a refactor that swaps `AuthedImage` for a plain `<img>` inside the real surface is caught, not just the shared component. |
