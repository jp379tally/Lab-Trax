...
### Protected Workflow Matrix — Desktop (32)
...
| 16 | Attachments / files / photos | desktop | `routes/cases-attachments.test.ts`, `routes/cases-prescription-photo.test.ts` (server) ; `src/components/__tests__/AuthedMedia.attachment.test.tsx` + `src/pages/__tests__/attachment-row-authed-media.test.tsx` (desktop — image attachments render via authenticated media against canonical `/attachments/:id/file`, never a plain `<img src>` at the protected URL; covers the shared `AuthedImage` component **and** every real shipped surface: `AttachmentThumb`, `HistoryEventMedia`, the full-size `MediaLightbox`, and the `AttachmentRow` Files-tab list row) | Yes — API + desktop tests |
...
| 26 | Desktop sidebar navigation order — NAV (Dashboard, Cases) and SECONDARY (Subscription, Admin Settings group, Maintenance, Download Desktop App) appear in approved order with no two-factor/2FA labels inserted | desktop | `src/__tests__/nav-ordering.test.ts` | Yes — desktop tests |
| 27 | Desktop update panel status display — Settings → Desktop app card (AppVersionCard) shows correct badge (idle/checking/available/downloading/downloaded/not-available/error), Restart & install button when downloaded, and dev-build note when autoUpdaterEnabled is false; panel visible to all users (not only behind transient UpdateBanner) | desktop | `src/pages/__tests__/update-ipc-contract.test.tsx`, `src/pages/__tests__/settings-desktop-panel.test.tsx` | Yes — desktop tests |
| 28 | Desktop Settings phone-verification security status — Profile tab shows a Security card with phone-verification state (Verified/Not verified/No phone on file); no "Two-factor authentication" or "2FA" wording anywhere on the Settings page | desktop | `src/pages/__tests__/settings-security-status.test.tsx` | Yes — desktop tests |
| 29 | Desktop lab-logo authenticated rendering — practiceLogoUrl from /api/auth/me always goes through AuthedImage → authedMediaFetch → blob URL; no raw <img src={practiceLogoUrl}> that would 401 silently | desktop + API | `src/pages/__tests__/settings-lab-logo-authed.test.tsx` (desktop: authedMediaFetch called, raw URL never in DOM, blob URL rendered) ; `src/routes/auth-me-desktop-contract.test.ts` (API: practiceLogoUrl, phoneVerifiedAt, emailVerifiedAt present + correct shape in GET /api/auth/me response) | Yes — desktop + API tests |
| 30 | Customer window 4-tab bar — the customer slide-in window (`PracticeEditor`, opened from the Accounts page) shows all 4 tabs (Basic Info, Invoices, Statements, Card on File); switching to Invoices/Statements/Card on File mounts each tab's real content fed by its own endpoint (`/invoices?practiceId=`, `/invoices/practice-statements?providerOrganizationId=&labOrganizationId=`, Card on File scaffold + authorization-form buttons); switching back to Basic Info keeps the form intact; Basic Info "Save changes" still PATCHes `/organizations/:id` (accountNumber kept for provider-with-parent-lab), closes on success, and surfaces errors without closing | desktop | `src/pages/__tests__/practices-customer-window-tabs.test.tsx` | Yes — desktop tests |
| 31 | Bulk invoice delete + Reset to $0 — `DELETE /api/invoices/bulk` and `POST /api/invoices/bulk-reset` handle selections spanning multiple lab orgs AND legacy mobile invoices (`mobile:<id>` synthesized from `lab_cases` blobs): reset zeroes the blob in place (price 0, status draft, prior price preserved) so the invoice still surfaces at $0.00; `all:true` also sweeps legacy invoices in the lab org; unmatched ids reported via `skippedCount`; audit rows written for legacy resets; cross-org access fails closed (count 0, blob untouched) | desktop + API | `routes/invoices-bulk-multi-org.test.ts` (15 tests, DB-gated on `DATABASE_URL`) | Yes — API tests |
| 32 | Shift-click range selection on multi-select lists — a normal checkbox click toggles one row and moves the anchor; a subsequent shift-click selects every row between the anchor and the clicked row in the currently visible (filtered + sorted) order, keeps prior selections outside the range, and falls back to a single toggle + anchor reset when the anchor is no longer visible; shift-click never starts a native text selection. Shared helpers (`computeShiftClickRange`, `shiftKeyFromChangeEvent`, `suppressShiftClickTextSelection` in `src/lib/shift-click-range.ts`) back all nine lists so a helper refactor cannot silently break any screen | desktop | `src/lib/__tests__/shift-click-range.test.ts` (helper unit tests: range down/up, adjacent, whole-list, anchor === clicked, no anchor, anchor/clicked not visible, empty list, no-mutation, shiftKey read, text-selection suppression) ; per-screen integration tests: `src/pages/__tests__/cases-shift-click-range.test.tsx`, `invoices-shift-click-range.test.tsx`, `doctors-shift-click-range.test.tsx`, `accounts-shift-click-range.test.tsx`, `statements-shift-click-range.test.tsx`, `receive-payments-shift-click-range.test.tsx`, `settings-deleted-cases-shift-click-range.test.tsx`, `src/pages/finance/__tests__/make-deposits-shift-click-range.test.tsx`, `register-reconcile-shift-click-range.test.tsx` | Yes — desktop tests |
...
| Layer | File | What it guards |
|-------|------|----------------|
| Desktop unit | `artifacts/labtrax-desktop/src/components/__tests__/AuthedMedia.attachment.test.tsx` | Case-attachment photos render through `AuthedImage` — the shared component behind every desktop attachment surface (AttachmentRow, Files tab, image lightbox, history-event thumbnails, PrescriptionPreview). Layers: **(A) the shared `AuthedImage` component** guards that it (1) fetches the canonical bearer-auth-gated `GET /api/cases/:caseId/attachments/:attachmentId/file` via `authedFetch` and renders the resulting object URL; (2) the rendered `<img src>` is the blob object URL, **never** the raw protected `/file` URL (a plain `<img src="…/file">` would 401 and render blank — the stale-desktop-build bug this locks against); (3) the fallback (not an `<img>`) renders when the authenticated fetch fails. **(B) the real shipped surfaces** mount the actual components users see and assert each builds the canonical `/file` URL and renders it *through* `AuthedImage` (object-URL `<img>`, never a plain `<img src="…/file">`, plus the "Unavailable"/blank fallback on fetch failure), so a refactor that swaps `AuthedImage` for a plain `<img>` inside any one surface is caught, not just the shared component: **`AttachmentThumb`** (Files-tab thumbnail) and **`HistoryEventMedia`** (history-timeline inline thumbnail) from `PrescriptionPreview.tsx`, and the full-size **`MediaLightbox`** from `AuthedMedia.tsx` (the shared image/video lightbox used by both `cases.tsx` and `PrescriptionPreview.tsx`). |
| Desktop unit | `artifacts/labtrax-desktop/src/pages/__tests__/attachment-row-authed-media.test.tsx` | The real Files-tab list-row surface **`AttachmentRow`** (from the heavy `cases.tsx` page module — isolated in its own file for the jspdf/react-pdf mocks) renders its image thumbnail *through* `AuthedImage` against the canonical `/file` endpoint (object-URL `<img>`, never a plain `<img src="…/file">`, plus the blank fallback on auth failure). |

Run command:
```
pnpm --filter @workspace/labtrax-desktop exec vitest run src/components/__tests__/AuthedMedia.attachment.test.tsx src/pages/__tests__/attachment-row-authed-media.test.tsx
```
